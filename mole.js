...
function popUpRandomMole() {
  ...
  const moleHeads = document.querySelectorAll('.wgs_head');
  ...
  moleHead.classList.remove('wgs_head-hidden', 'wgs_head-whacked');
...
function hideMole(mole) {
  clickable = false;
  mole.classList.add('wgs_head-hidden');
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  for (let moleHead of moleHeads) {
    moleHead.addEventListener('click', event => {
      ...
      // UNCOMMENT THIS LINE OF CODE WHEN DIRECTED
      event.target.classList.add('wgs_head-hidden');

      // UNCOMMENT THIS LINE OF CODE WHEN DIRECTED FOR THE BONUS
      event.target.classList.add('wgs_head-whacked');
    });
  }
});
